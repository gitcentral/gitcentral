...
/***/ function(module, exports) {
	...
	treeJSON = d3.json("flare.json", function (error, treeData) {
	     ...
	     */
	    function addParents() {
	        var node = arguments.length <= 0 || arguments[0] === undefined ? root : arguments[0];
	    ...
	    function mergeBranches() {
	        ...
	        smartVisit(root, function (outerNode) {
	            smartVisit(root, function (innerNode) {
	                if (outerNode !== innerNode && outerNode.name === innerNode.name) {
	                    //add innerNode's parents to outerNode's parents array
	                    if (outerNode.parents) {
	                        var _outerNode$parents;

	                        (_outerNode$parents = outerNode.parents).push.apply(_outerNode$parents, _toConsumableArray(innerNode.parents));
	                    }

	                    //add outerNode to each of innerNode's parents'
	                    //children arrays
	                    if (innerNode.parents) {
	                        innerNode.parents.forEach(function (parent) {
	    ...
	    function setFalse(node, property) {
	        ...
	        if (node.children) {
	            node.children.forEach(function (child) {
	                return setFalse(child, property);
	            });
	        }
	    }
	    ...
	    function checkLevels() {
	        var startNode = arguments.length <= 0 || arguments[0] === undefined ? root : arguments[0];

	        var edited = false;
	        smartVisit(startNode, function (node) {
	            if (node === root) {
	                if (node.level !== 1) {
	                    node.level = 1;
	                    edited = true;
	                ...
	                var parentLevels = node.parents.map(function (parent) {
	                    return parent.level || 0;
	                });
	                var correctLevel = Math.max.apply(Math, _toConsumableArray(parentLevels)) + 1;
	    ...
	    function setLevels() {
	        ...
	        smartVisit(startNode, function (node) {
	            if (!node.parents) {
	                ...
	                node.level = Math.max.apply(Math, _toConsumableArray(parentLevels)) + 1;
	            }
	        });
	    ...
	    function update(source) {
	        ...
	        function filterDuplicates(nodesWithDupes) {
	            ...
	            nodesWithDupes.forEach(function (node) {
	                var exists = false;
	                nodes.forEach(function (alreadyPresent) {
	        ...
	        var nodesWith2Parents = nodes.filter(function (node) {
	        ...
	        var multiParentsWithDupes = [];

	        nodesWith2Parents.forEach(function (node) {
	        ...
	        multiParentsWithDupes.forEach(function (linkObj) {
	            var exists = false;
	            multiParents.forEach(function (item) {
...
/***/ function(module, exports) {
	 ...
	 */
	function timestamp() {
	  var date = new Date(2016, 8, nextDay(), int(24), int(60), int(60));
	...
	function makeConfig(branch) {
	  return {
	    ...
	    gitCommands: timestamp() + '\n\nPossible git commands:\n git checkout [branch name]\n   options:\n   -b: create and check out new branch\n git branch [branch name]\n   options:\n   -d: delete branch\n   -D: delete branch, suppress warnings\n git tag [tag name]'
	  };
	}

	function addGitCommands(commitConfig) {
	  for (var _len = arguments.length, commands = Array(_len > 1 ? _len - 1 : 0), _key = 1; _key < _len; _key++) {
	    commands[_key - 1] = arguments[_key];
	  }

	  commands.forEach(function (command) {
	    return commitConfig.gitCommands += '\n ' + command;
	  });
	  return commitConfig;
	}

	//Initial setup of graph
	var author = 'Arnav & Cady';
	var graphConfig = {
	  template: "metro",
	  orientation: "horizontal",
	  mode: "compact" };

	var tailNodeCommands = ['git reset HEAD(~[n]), [n] = number of commits to reset\n   options:\n   --hard: obliterate last n commits (can\'t be undone)\n   --soft: remove last n commits but leave working\n           directory unchanged', 'git merge', 'git rebase', 'git pull'];

	var gitGraph = new GitGraph(graphConfig);
	gitGraph.author = author;

	// Create a master branch with some commits
	var master = gitGraph.branch("master"); //grey
	master.commit(makeConfig(master)).commit(makeConfig(master)).commit(makeConfig(master));

	//Create a dev branch with some commits
	var dev = master.branch('dev'); //blue
	dev.commit(makeConfig(dev)).commit(makeConfig(dev)).commit(makeConfig(dev)).commit(makeConfig(dev));

	//Add commits to master
	master.commit(makeConfig(master)).commit(makeConfig(master));

	//Merge dev into master. Commit more onto dev.
	dev.merge(master, makeConfig(master));
	dev.commit(makeConfig(dev)).commit(makeConfig(dev)).commit(makeConfig(dev));

	//Create a new branch from master, with some commits
	var yellow = master.branch("Should be yellow"); //yellow (l0l)
	yellow.commit(makeConfig(yellow)).commit(makeConfig(yellow)).commit(addGitCommands.apply(undefined, [makeConfig(yellow)].concat(tailNodeCommands)));

	//Commit on dev some more
	dev.commit(makeConfig(dev)).commit(makeConfig(dev)).commit(makeConfig(dev));

	//Merge dev into master
	dev.merge(master, addGitCommands.apply(undefined, [makeConfig(master)].concat(tailNodeCommands)));
